...
class AC_Net:
    ...
    def __init__(self, scope, trainer, x_dim_p, x_dim_v, n_actions, num_layers, layer_size, regu_scalar):
        ...
        self.regularizer = slim.l2_regularizer(self.regu_scalar, scope=None)

        self.w_initializer, self.b_initializer = tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)

        self.Input_p = tf.compat.v1.placeholder(tf.float32, shape=[None, self.x_dim_p], name='policy_input')
        self.Input_v = tf.compat.v1.placeholder(tf.float32, shape=[None, self.x_dim_v], name='value_input')
